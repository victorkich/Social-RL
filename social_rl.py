...
class Environment:
    ...
    def get_state(self):
        ...
        for i in range(4):
            if i == 0 and any(active_locks[[0, 2]]):
                c_lock = 140
            elif i == 1 and any(active_locks[[0, 1]]):
                c_lock = 140
        ...
        for i in range(4):
            if self.carried_coins[i]:
                self.x_coins[i] = agents[self.carried_coins[i] - 1].x
                self.y_coins[i] = agents[self.carried_coins[i] - 1].y
    ...
    def release_event(self, id):
        for i in range(4):
            if self.carried_coins[i] == id + 1:
                self.carried_coins[i] = False
                agents[id].grabbed_coin = False
                self.x_coins[i] = agents[id].x
                self.y_coins[i] = agents[id].y
...
class Agent:
    def __init__(self, id, x_start, y_start, radius, color, random_seed=2023, hidden_size=128, state_size=81, action_size=3, action_prior="uniform"):
        self.id = id
        self.x = x_start
        self.y = y_start
        self.radius = radius
